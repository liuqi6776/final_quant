...
PORTFOLIO_SIZE = 10
HOLDING_DAYS = 20
MAX_WEIGHT_PER_INDUSTRY = 0.40  # 集中持股时行业上限略微放宽
...
def simulate_strategy_stop_loss(df_by_date, trade_dates, rebalance_dates, use_stop_loss=False, use_take_profit=False, sl_thresh=-0.08, tp_thresh=0.15):
    ...
    for idx, dt in enumerate(trade_dates):
        ...
        # Execute stop-loss/take-profit liquidations on next day's open
...
def run_top10_stop_loss_analysis():
    ...
    test_cases = [
        ...
        ('3. SL & TP (-8% SL / +15% TP)', True, True, -0.08, 0.15),
        ('4. Stop-Loss Only (-10% SL)', True, False, -0.10, 0.0),
        ('5. SL & TP (-10% SL / +20% TP)', True, True, -0.10, 0.20),
        ('6. Tight Stop-Loss (-5% SL)', True, False, -0.05, 0.0)
    ]
    ...
    print("\n==========================================================================")
    print("【TOP-10 PORTFOLIO STOP-LOSS / TAKE-PROFIT SENSITIVITY SUMMARY (2021-2026)】")
    print("==========================================================================")
    ...
    for i, year in enumerate(years):
        ...
        for name, _, _, _, _ in test_cases:
            col_name = name.split(". ")[1].replace(" (No SL / No TP)", "").replace(" Only", "").replace(" & TP", "").replace("SL & TP ", "")
