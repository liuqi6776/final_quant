...
# 将项目根目录（含有 processing 的地方）加入 sys.path
...
def preprocess_factors(df, feature_cols):
    ...
    3. Industry, Size, and Beta Neutralization (Residuals of regression)
    """
    from processing.cleaner import remove_outliers_mad, calculate_rolling_beta, neutralize_factors
    print("Preprocessing factors cross-sectionally (MAD winsorization, Standardization, Neutralization)...", flush=True)
    df = df.copy()
    
    # 1. Winsorize using MAD method
    print("  Applying MAD Winsorization...", flush=True)
    df = remove_outliers_mad(df, feature_cols, n=3.0)
    ...
    df[feature_cols] = (df[feature_cols] - date_means) / (date_stds + 1e-8)
    
    # 3. Neutralization (Industry + Size + Beta)
    print("  Applying Industry, Size & Beta Neutralization...", flush=True)
    if 'beta' not in df.columns:
        df = calculate_rolling_beta(df)
    df = neutralize_factors(df, feature_cols)
...
def train_and_predict(feature_config='B', output_file=OUTPUT_FILE, start_month='202101'):
    t0 = time.time()
    ...
    # --- 动态计算微观结构因子 (资金流与筹码分布) ---
    print("正在动态计算新引入的资金流和筹码分布因子...", flush=True)
    # A. 筹码成本均线 (TMA Cost)
    df = df.sort_values(['ts_code', 'trade_date']).reset_index(drop=True)
    close_vals = df['close'].values
    turnover_vals = df['turnover_rate'].fillna(0.0).values / 100.0
    ...
    for i in range(len(boundaries) - 1):
        ...
        c_slice = close_vals[start:end]
        t_slice = turnover_vals[start:end]
        cost_slice = np.zeros(end - start)
        cost_slice[0] = c_slice[0]
        for j in range(1, len(c_slice)):
            cost_slice[j] = cost_slice[j-1] * (1.0 - t_slice[j]) + c_slice[j] * t_slice[j]
    ...
    # 清除临时列
    df = df.drop(columns=['roll_high_60d', 'roll_low_60d', 'chip_cost_tma'])
    # ----------------------------------------------
    ...
    micro_cols = ['moneyflow_large', 'chip_concentr_60d', 'chip_profit_dev']
    
    if feature_config == 'A':
        # Baseline: 排除所有新引入的 Vibe 因子和微观结构因子
        extra_excludes.update(vibe_cols)
        extra_excludes.update(micro_cols)
    elif feature_config == 'B':
        # Baseline + Vibe + Microstructure: 包含所有因子
        pass
    else:
    ...
    for month in pred_months:
        ...
        test_df = df.loc[test_mask, feature_cols + ['trade_date', 'ts_code', 'open', 'next_open', 'close', 'pct_chg', 'industry', 'ret_20d', 'mkt_excess_ret_20d']].copy()
        ...
        all_preds.append(test_df[['trade_date', 'ts_code', 'open', 'next_open', 'close', 'pct_chg', 'industry', 'ret_20d', 'mkt_excess_ret_20d', 'pred_score']])
