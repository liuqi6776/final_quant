...
def run_attribution():
    ...
    df_feat = pd.read_parquet(FEATURES_FILE, columns=['trade_date', 'ts_code', 'pct_chg', 'circ_mv', 'industry', 'pb', 'close'])
    ...
    df_feat['pb'] = pd.to_numeric(df_feat['pb'], errors='coerce').fillna(1.0)
    df_feat['close'] = pd.to_numeric(df_feat['close'], errors='coerce').fillna(0.0)
    ...
    # C. 价值因子 (HML - High Minus Low)
    print("Calculating daily HML (Value) factor based on 1/PB...", flush=True)
    df_feat['bm'] = 1.0 / (df_feat['pb'] + 1e-8)
    def calc_hml(group):
        group = group[group['bm'] > 0]
        n = len(group)
        if n < 10:
            return 0.0
        sorted_g = group.sort_values('bm')
        n_cutoff = int(n * 0.3)
        r_value = sorted_g.iloc[-n_cutoff:]['pct_chg'].mean()  # 高 BM (Value)
        r_growth = sorted_g.iloc[:n_cutoff]['pct_chg'].mean()  # 低 BM (Growth)
        return r_value - r_growth

    df_hml = df_feat.groupby('trade_date').apply(calc_hml).reset_index().rename(columns={0: 'HML'})

    # D. 动量因子 (UMD - Up Minus Down)
    print("Calculating daily UMD (Carhart Momentum) factor...", flush=True)
    df_feat = df_feat.sort_values(['ts_code', 'trade_date']).reset_index(drop=True)
    grouped_close = df_feat.groupby('ts_code')['close']
    ret_12m = (df_feat['close'] - grouped_close.shift(252)) / (grouped_close.shift(252) + 1e-8)
    ret_1m = (df_feat['close'] - grouped_close.shift(21)) / (grouped_close.shift(21) + 1e-8)
    df_feat['umd_score'] = ret_12m - ret_1m
    
    def calc_umd(group):
        group = group[group['umd_score'].notna()]
        n = len(group)
        if n < 10:
            return 0.0
        sorted_g = group.sort_values('umd_score')
        n_cutoff = int(n * 0.3)
        r_winners = sorted_g.iloc[-n_cutoff:]['pct_chg'].mean()  # 高动量 (Winners)
        r_losers = sorted_g.iloc[:n_cutoff]['pct_chg'].mean()   # 低动量 (Losers)
        return r_winners - r_losers

    df_umd = df_feat.groupby('trade_date').apply(calc_umd).reset_index().rename(columns={0: 'UMD'})
    ...
    df_reg = df_reg.merge(df_hml, on='trade_date', how='inner')
    df_reg = df_reg.merge(df_umd, on='trade_date', how='inner')
    ...
    # A. Model 1: 考虑 市场(R_m) + 市值(SMB) + 价值(HML) + 动量(UMD)
    X1 = df_reg[['R_m', 'SMB', 'HML', 'UMD']]
    X1 = sm.add_constant(X1)
    model1 = sm.OLS(y, X1).fit()
    
    # B. Model 2: 考虑 市场(R_m) + 市值(SMB) + 价值(HML) + 动量(UMD) + 行业超额因子 (控制了多重共线性)
    X2 = df_reg[['R_m', 'SMB', 'HML', 'UMD'] + ind_cols]
    ...
    report.append("\n[Model 1: Fama-French & Carhart Factor Regression]")
    report.append(f"Formula: R_strategy = alpha + beta_m * R_m + beta_s * SMB + beta_h * HML + beta_u * UMD + e")
    report.append("--------------------------------------------------------------------------")
    ...
    beta_h_m1 = model1.params['HML']
    beta_u_m1 = model1.params['UMD']
    ...
    report.append(f"Beta Value (beta_h):     {beta_h_m1:.4f} (t-stat: {model1.tvalues['HML']:.2f})")
    report.append(f"Beta Momentum (beta_u):  {beta_u_m1:.4f} (t-stat: {model1.tvalues['UMD']:.2f})")
    report.append(f"R-squared:               {r2_m1:.4f} (Style explains {r2_m1:.2%} of variance)")
    
    report.append("\n==========================================================================")
    report.append("[Model 2: Fama-French, Carhart & Orthogonalized Industry Excess Regression]")
    report.append(f"Formula: R_strategy = alpha + beta_m * R_m + beta_s * SMB + beta_h * HML + beta_u * UMD + sum(beta_i * (R_ind_i - R_m)) + e")
    report.append("--------------------------------------------------------------------------")
    ...
    beta_h_m2 = model2.params['HML']
    beta_u_m2 = model2.params['UMD']
    ...
    report.append(f"Beta Value (beta_h):     {beta_h_m2:.4f} (t-stat: {model2.tvalues['HML']:.2f})")
    report.append(f"Beta Momentum (beta_u):  {beta_u_m2:.4f} (t-stat: {model2.tvalues['UMD']:.2f})")
