...
def simulate_strategy(df_by_date, trade_dates, portfolio_size, holding_days, max_weight_per_industry=0.25):
...
def run_holding_sizes_analysis():
    ...
    # Test configurations: (portfolio_size, holding_days)
    configs = [
        (100, 20, 0.20),
        (100, 10, 0.20),
        (10, 20, 0.40),
    ...
    for size, h, max_w in configs:
        name = f"Top{size}_Hold{h}d"
        print(f"Running backtest for: {name}...", flush=True)
        nav_df = simulate_strategy(df_by_date, trade_dates, size, h, max_w)
        ...
        full_metrics.append({
            'Portfolio Size': f"Top {size}",
            'Holding Days': f"{h} Days",
    ...
    print("\n==========================================================================")
    print("【HOLDING FREQUENCY SENSITIVITY FOR TOP-10 vs TOP-100 (2021-2026)】")
    print("==========================================================================")
    ...
    for i, year in enumerate(years):
        row = {'Year': year}
        for size, h, _ in configs:
            name = f"Top{size}_Hold{h}d"
            r = yearly_navs[name].iloc[i]
            row[name] = f"{r['Return']*100:.1f}% ({r['MaxDD']*100:.1f}%)"
        table_rows.append(row)
        
    print(pd.DataFrame(table_rows).to_markdown(index=False))

if __name__ == '__main__':
    run_holding_sizes_analysis()
