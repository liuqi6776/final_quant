...
def simulate_strategy_scaled(df_by_date, trade_dates, rebalance_dates, filter_type='none'):
    """
    Simulates portfolio strategy with scaled positions.
    filter_type:
      - 'none': Always 100% stock
      - 'ma20_hard': close < MA20 -> 0% stock, else 100%
      - 'ma20_3stage': close > MA20*1.02 -> 100%, 0.98<=close/MA20<=1.02 -> 50%, <0.98 -> 0%
      - 'ma20_5stage': close > MA20*1.03 -> 100%, 1.01<ratio<=1.03 -> 75%, 0.99<=ratio<=1.01 -> 50%, 0.97<=ratio<0.99 -> 25%, <0.97 -> 0%
      - 'ma20_3stage_er': 3-stage scaled position combined with Kaufman's Efficiency Ratio (ER) as ADX-equivalent
    """
    ...
    for idx, dt in enumerate(trade_dates):
        ...
        if len(day_data) > 0:
            ...
            er = day_data['index_er'].iloc[0] if 'index_er' in day_data.columns else 0.5
            
            if filter_type == 'ma20_hard':
                weight_factor = 0.0 if ratio < 1.0 else 1.0
            elif filter_type == 'ma20_3stage':
                if ratio > 1.02:
                    weight_factor = 1.0
                elif ratio >= 0.98:
                    weight_factor = 0.5
                ...
                if ratio > 1.03:
                    ...
                    weight_factor = 0.0
            elif filter_type == 'ma20_3stage_er':
                # Kaufman ER < 0.35 indicates range-bound (震荡市), we hold a floor of 50% position
                if er < 0.35:
                    weight_factor = 0.50
                else:
                    # Strong trend market, follow normal 3-stage
                    if ratio > 1.02:
                        weight_factor = 1.0
                    elif ratio >= 0.98:
                        weight_factor = 0.5
...
def run_scaled_analysis():
    ...
    # Calculate Kaufman's Efficiency Ratio (ER) as ADX-equivalent
    idx_df['abs_change_10d'] = (idx_df['close'] - idx_df['close'].shift(10)).abs()
    idx_df['sum_abs_daily_change_10d'] = (idx_df['close'] - idx_df['close'].shift(1)).abs().rolling(10).sum()
    idx_df['index_er'] = idx_df['abs_change_10d'] / idx_df['sum_abs_daily_change_10d']
    idx_df['index_er'] = idx_df['index_er'].fillna(0.5)
    
    ratio_map = idx_df.set_index('trade_date')['index_ratio'].to_dict()
    er_map = idx_df.set_index('trade_date')['index_er'].to_dict()
    trend_map = idx_df.set_index('trade_date')['downtrend'].to_dict()
    
    df['index_ratio'] = df['trade_date'].map(ratio_map).fillna(1.0)
    df['index_er'] = df['trade_date'].map(er_map).fillna(0.5)
    ...
    test_cases = [
        ('1. Pure Selection (No Wind-down)', 'none'),
        ('2. MA20 Hard (Hard Wind-down)', 'ma20_hard'),
        ('3. MA20 3-Stage Scaled (100/50/0)', 'ma20_3stage'),
        ('4. MA20 5-Stage Scaled (100/75/50/25/0)', 'ma20_5stage'),
        ('5. MA20 3-Stage + Kaufman ER (ADX-eq)', 'ma20_3stage_er'),
    ]
    
    full_metrics = []
    yearly_navs = {}
    
    for name, f_type in test_cases:
        print(f"Running backtest for: {name}...", flush=True)
        nav_df = simulate_strategy_scaled(df_by_date, trade_dates, rebalance_dates, f_type)
    ...
    print("\n==========================================================================")
    print("【SCALED POSITION WIND-DOWN TIMING STRATEGY SUMMARY (2021-2026)】")
    print("==========================================================================")
    ...
    for i, year in enumerate(years):
        ...
        for name in yearly_navs:
            col_name = name.split(". ")[1].replace(" (No Wind-down)", "").replace(" (Hard Wind-down)", "").replace(" Scaled", "").replace(" + Kaufman ER (ADX-eq)", " (ER)")
